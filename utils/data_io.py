...
# --舊做法（Keras風格）
# --當前已經改用PyTorch的DataLoader搭配TensorDataset + create_sliding_window這樣的處理方式，因此這段舊的Keras風格的ReccurentTrainingGenerator就可以不用了。
...
# -- ⚠️ 已改用 PyTorch tensor + DataLoader 預測，此類別已不再使用。
# -- 目前的預測流程已經不使用 Sequence（Keras 專用）風格的 generator，而是改用PyTorch的DataLoader、tensor以及sliding window處理方法。
# -- 此類別是舊版Keras 風格（tf.keras.utils.Sequence），在轉向PyTorch後就不再需要保留。
